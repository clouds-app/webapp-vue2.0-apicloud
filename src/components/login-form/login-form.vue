<template>
    ...
    <van-cell-group>
            ...
            <van-cell-group>
                <van-button :disabled="disabledLogin" :loading="loadingSubmit" loading-txt="提交中.." @click="handleLogin()" round  long type="primary" size="large">登 陆</van-button>
            </van-cell-group>
...
export default {
    ...
    computed:{ //只能计算DATA中的属性数据
        watchLoginForm(){
            if(this.loginForm.userName.length > 0 && this.loginForm.phone.length > 0 && this.loginForm.licenseKey.length > 0){
                return true
            }else{
                return false
            }
        ...
        handleLogin(){
            ...
            if(this.checkInputData()){
               ...
               this.$store.dispatch('handleLogin',params).then(res=>{
                     ...
                     switchMethods.turnToPage('home')
                    _self.$toast.success('登陆成功');
               }).catch(err=>{
                   _self.loadingSubmit=false 
                  _self.$toast.fail('抱歉，请再来一次');
                  
                 switchMethods.turnToPage('home') //测试跳转
               })
